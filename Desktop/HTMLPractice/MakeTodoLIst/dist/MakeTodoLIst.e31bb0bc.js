...
exports.saveTodoList = saveTodoList;
exports.getTodoList = getTodoList;
exports.deleteTodoListLocal = deleteTodoListLocal;
exports.deleteAllLocal = deleteAllLocal;
exports.saveFinishID = saveFinishID;
exports.removeFinishID = removeFinishID;
...
function saveTodoList(state) {
  var now = getTime();
  localStorage.setItem("".concat(state.id), JSON.stringify({
    text: state.item,
    time: now
  }));
}
...
function saveFinishID(arr) {
  if (localStorage.getItem('finish') !== null) {
    arr = Array.from(new Set([].concat(_toConsumableArray(arr), _toConsumableArray(localStorage.getItem('finish').split(',')))));
...
function removeFinishID(id) {
  if (localStorage.getItem('finish') !== null) {
    var arr = localStorage.getItem('finish').split(',');
    arr = arr.filter(function (finish) {
      return finish !== id;
    });
    localStorage.removeItem('finish');
    localStorage.setItem('finish', arr);
...
function getTodoList(id) {
  return localStorage.getItem("".concat(id));
}

function deleteTodoListLocal(id) {
  localStorage.removeItem("".concat(id));
}

function deleteAllLocal() {
  localStorage.clear();
}
},{}],"remove.js":[function(require,module,exports) {
"use strict";
...
function deleteTodoList(state) {
  (0, _storage.deleteTodoListLocal)(state.id);
  (0, _storage.removeFinishID)(state.id);
  var box = document.getElementById("".concat(state.id));
  box.remove();
  checkClear();
}
...
},{"./storage":"storage.js"}],"create.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.makeTodoList = makeTodoList;
...
function makeTodoList(state) {
  ...
  if ((0, _storage.getTodoList)(state.id) !== null) {
    ...
    if ((0, _storage.getTodoList)('finish') && (0, _storage.getTodoList)('finish').includes(state.id)) {
...
function doneTodoList(state) {
  ...
  finishID = finishID.filter(function (element) {
    return temp.includes(element);
  });
  var findListBox = document.getElementById("".concat(state.id));

  if (findListBox.classList.contains('finish')) {
    findListBox.classList.remove('finish');
    finishID = finishID.filter(function (finish) {
    ...
    (0, _storage.saveFinishID)(finishID);
  }
}
},{"./storage":"storage.js","./remove":"remove.js"}],"index.js":[function(require,module,exports) {
"use strict";
...
//DOM 레퍼런스 작성
var main = document.querySelector('.glass'),
...
function showLocalStorage() {
  ...
  if (localStorage.length !== 0) {
    ...
    order.forEach(function (element) {
      return store.dispatch(ShowTodoList(element));
    });
  }
} //액션 정의


var showTodoList = 'showTodoList';
var addTodoList = 'addTodoList';
var removeTodoList = 'removeTodoList';
var finishTodoList = 'finishTodoList';
var removeAll = 'removeAll'; //액션 함수 정의

var ShowTodoList = function ShowTodoList(listID) {
  return {
    type: showTodoList,
    listID: listID
  };
...
var AddList = function AddList(listitem) {
  return {
    type: addTodoList,
    listitem: listitem
  };
...
var DoneList = function DoneList(listID) {
  return {
    type: finishTodoList,
    listID: listID
  };
  ...
  switch (action.type) {
    case showTodoList:
      return _objectSpread(_objectSpread({}, state), {}, {
        id: action.listID,
        item: JSON.parse(localStorage.getItem("".concat(action.listID))).text,
        add: true,
        remove: false,
        done: false,
        all: false,
...
var render = function render() {
  var state = store.getState(); //makeTodoList(state);
  ...
  if (add) {
    (0, _create.makeTodoList)(state);
  } else if (remove) {
    (0, _remove.deleteTodoList)(state);
  } else if (done) {
    (0, _create.doneTodoList)(state);
  } else if (all) {
    (0, _remove.deleteAll)();
  } else {}
};
...
todoBox.addEventListener('click', function (e) {
  if (e.target.id === 'remove') {
    var currentDeleteBnt = e.target.parentNode.id;
    store.dispatch(RemoveList(currentDeleteBnt));
  } else if (e.target.id === 'done') {
    var _currentDeleteBnt = e.target.parentNode.id;
    store.dispatch(DoneList(_currentDeleteBnt));
  }
}, false);
clear.addEventListener('click', function (e) {
  ...
  confirm.id = 'confirm';
  confirm.innerHTML += '<div>작성하신 ToDo 리스트를 정말 다 지우시겠어요? <br>맞다면 확인을, 아니라면 취소를 눌러주세요.</div>';
...
//# sourceMappingURL=/MakeTodoLIst.e31bb0bc.js.map